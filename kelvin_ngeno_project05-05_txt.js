...
function scrambleTiles() {
   for (let i = 0; i < allTiles.length; i++) {  // Fixed to use < instead of <=
      ...
      board.insertBefore(board.children[randomIndex], board.children[i]);  // Fixed insert method
   }
}
...
function playConcentration() {
      ...
      allTiles[i].onclick = function() {
         ...
         if (this.lastElementChild.className === "back") {  // Fixed 'This' to 'this' and = to ===
            ...
            if (tilesFlipped === 1) {  // Fixed = to ===
               ...
               timeID = window.setTimeout(flipBack, 1000);  // Fixed delay from 1 to 1000 ms
            }
         }
      }
   }  
   ...
   function flipBack() {
      ...
      tilesFlipped = 0;  // Fixed 'titlesFlipped' to 'tilesFlipped'
   }   
};